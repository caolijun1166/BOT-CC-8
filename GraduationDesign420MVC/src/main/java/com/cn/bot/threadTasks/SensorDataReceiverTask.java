		...
		while(true){
			...
			try{
				...
				while(true){
					...
					socket.setSoTimeout(60000);//设置读取数据超时时间,如果20秒内未读到数据，则会抛出SocketTimeoutException异常
					log.debug("-------------------------------客户机：" + data + "-------------------------------");
					
					//判断温度数据，并自动向单片机发送控制信号				
					if(data != null){
						tempDec = Integer.parseInt(data.split(" ")[5].split("00")[0], 16);
						log.debug("-------------------------------tempDec is：" + tempDec + "-------------------------------");
